...
public class PhoneNumberValidator : IValidator<string>
{
    ...
    public void Validate(string value)
    {
        if (string.IsNullOrWhiteSpace(value))
            throw new ArgumentNullOrWhiteSpaceException(ExceptionMessages.PHONE_NUMBER_NOT_NULL_OR_WHITE_SPACE, nameof(value));
        if (value.Length > MAX_LENGTH)
            throw new PhoneNumberLongValueException(value, MAX_LENGTH);        
        if (value.Length < MIN_LENGTH)
            throw new PhoneNumberShortValueException(value, MIN_LENGTH);
    }
}