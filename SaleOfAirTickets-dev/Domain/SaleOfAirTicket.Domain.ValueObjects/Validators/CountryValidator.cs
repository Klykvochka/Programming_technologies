...
/// </summary>
public class CountryValidator : IValidator<string>
{
    /// <summary>
    /// Максимальная длина.
    /// </summary>
    ...
    public void Validate(string value)
    {
        if (string.IsNullOrWhiteSpace(value))
            throw new ArgumentNullOrWhiteSpaceException(ExceptionMessages.COUNTRY_NOT_NULL_OR_WHITE_SPACE, nameof(value));
        if (value.Length > MAX_LENGTH)
            throw new CountryLongValueException(value, MAX_LENGTH);
        if (value.Length < MIN_LENGTH)
            throw new CountryShortValueException(value, MIN_LENGTH);
    }
}
