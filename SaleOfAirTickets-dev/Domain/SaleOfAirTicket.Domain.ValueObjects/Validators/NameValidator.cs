...
public class NameValidator : IValidator<string>
{
    ...
    public void Validate(string value)
    {
        if (string.IsNullOrWhiteSpace(value))
            throw new ArgumentNullOrWhiteSpaceException(ExceptionMessages.NAME_NOT_NULL_OR_WHITE_SPACE, nameof(value));
        if (value.Length > MAX_LENGTH)
            throw new NameLongValueException(value, MAX_LENGTH);
        if (value.Length < MIN_LENGTH)
            throw new NameShortValueException(value, MIN_LENGTH);
    }
}
