...
public class CityValidator : IValidator<string>
{
    ...
    public void Validate(string value)
    {
        if (string.IsNullOrWhiteSpace(value))
            throw new ArgumentNullOrWhiteSpaceException(ExceptionMessages.CITY_NOT_NULL_OR_WHITE_SPACE, nameof(value));
        if (value.Length > MAX_LENGTH)
            throw new CityLongValueException(value, MAX_LENGTH);
        if (value.Length < MIN_LENGTH)
            throw new CityShortValueException(value, MIN_LENGTH);
    }
}
